...
(defun dctrl-fastboot-action-flash (&optional type file)
  (let* ((type (or type (ido-completing-read "Type of flash: " (mapcar 'car dctrl-fastboot-flash-alist) nil t)))
    ...
    (unless (and file (file-exists-p file))
      (let* ((file-path (expand-file-name (concat (if aosp-out-path aosp-out-path)
...
(defun dctrl-fastboot-action-boot (&optional file)
  (let* ((file file)
	 tramp-cmd ctrlhost-filename)
    (unless (and file (file-exists-p file))
      (setq file (ido-read-file-name "Boot image: " (if aosp-out-path aosp-out-path) "boot.img" t)))
    (multiple-value-setq (tramp-cmd ctrlhost-filename)
      (dctrl-untramp-file file))
    (append tramp-cmd
...
(defun dctrl-fastboot-action-flash-raw (&optional kernel ramdisk)
  (let ((kernel (or kernel (ido-read-file-name "Kernel file: " (if aosp-out-path aosp-out-path) "kernel" t)))
	(ramdisk (or ramdisk (ido-read-file-name "Ramdisk file: " (if aosp-out-path aosp-out-path) "ramdisk.img" t)))
